...
##########################################################################################################################################
           if(r <= 0.8 - lambda_rho1):
              fullGrid[i][j][k] = fullGrid[i][j][k]
           elif((r >= 0.8 - lambda_rho1 and r <= 0.8 + lambda_rho1)):
              fullGrid[i][j][k] = (fullGrid[i][j][k] + rho_min)/2 + (fullGrid[i][j][k] - rho_min)/2*np.sin((0.8-r) * np.pi/(2*lambda_rho1))
           else:
              fullGrid[i][j][k] = rho_min
...
field_writes = solver.evaluator.add_file_handler('fields', max_writes=500, sim_dt = output_cadence, mode='overwrite')
...
load_writes = solver.evaluator.add_file_handler('load_data', max_writes=500, sim_dt = output_cadence, mode='overwrite')
...
try:
    ...
    while solver.ok and good_solution:
        ...
        if (solver.iteration-1) % 1 == 0:
            ...
            np.clip(lnrho['g'], -6.9, np.inf, out=lnrho['g'])
            np.clip(T['g'], 0.001, np.inf, out=T['g'])
except:
